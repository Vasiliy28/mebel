        ...
        @if(isset($menu))
            <menu class="flex-column nav nav-pills mt-0 pr-3 pl-5">
                @foreach($menu as $slag => $item)
                    <li class="nav-item">
                        <a class="nav-link mebel-nav-link {{request()->is('our_work/' . $slag) ? 'active bg-teal' :'text-muted'}} rounded-0"
                           href="{{route('our_work',['type' => $slag], FALSE)}}">
                            {{$item['title']}}</a>
                    </li>
                @endforeach
            </menu>
